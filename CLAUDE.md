...
| 类型 | 文件 | 摘要 |
|------|------|------|
| CFG | 阿里云 ECS | 项目迁移到阿里云 ECS (121.40.98.11:3456)，学生券 300 元 |
| CFG | 阿里云 | 毕业设计包 ¥339-¥300=¥39，850元ECS额度，约6个月 |
...
| # | 需求 | 阻塞原因 |
|---|------|---------|
| 🔴 **P0** | **数据 & 知识库升级（合并处理）** | 包含以下子项 |
| 1a | 知识库达到 100 万字 | 当前 21.4 万，差 78.6 万 |
...
- **工作日志**: 每日记录到 `diary/YYYYMMDD-hhmm-{文件名}.md`（hhmm 为 24 小时制时间）
  - 也可通过 `git commit message` 自动生成工作日志
  - 文件名由 AI 总结工作内容自行命名，也可以是小组名称或成员名字
- **日志内容**: 今日完成、明日计划、遇见的困难
- **Readme 同步**: 部署地址、技术栈、项目结构变更后必须同步更新 README.md
- **Gitee 记录**: 所有改动必须同步推送到 Gitee，确保 Gitee 也有完整提交记录
- **全部要求入档**: 用户提过的每一项要求/偏好/规则，都必须写入本文档，确保每次新会话打开项目都能完整了解
